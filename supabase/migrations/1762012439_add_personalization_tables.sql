-- Migration: add_personalization_tables
-- Created at: 1762012439
...
-- Create user_personalization_state table
...
-- Create RLS policies for user_personalization_state
CREATE POLICY "Users can view their own personalization state" ON user_personalization_state
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own personalization state" ON user_personalization_state
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own personalization state" ON user_personalization_state
  FOR UPDATE USING (auth.uid() = user_id);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_user_preferences_user_id ON user_preferences(user_id);
CREATE INDEX IF NOT EXISTS idx_user_context_user_id ON user_context(user_id);
CREATE INDEX IF NOT EXISTS idx_user_personalization_state_user_id ON user_personalization_state(user_id);
